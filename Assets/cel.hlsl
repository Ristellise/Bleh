...
void hard_shading(vertexOutput IN,
		    float3 LightColor,
		    float3 Nn,
		    float3 Ln,
		    float3 Vn,
...
float4 std_PS(vertexOutput IN) : COLOR {
	...
	hard_shading(IN,Lamp0Color, Nn, Ln, Vn, diffContrib, specContrib);
    //float3 diffuseColor = tex2D(ColorSampler, IN.UV).rgb;
	float3 diffuseColor = float3(0.75, 0.649, 0.50);
	//diffuseColor -= float3(0.50, 0.50, 0.50);
